...
public class GeneralB : Photon.PunBehaviour
{
    [SerializeField]
    Cards Cards;
    ...
    {
        if (Cards.General_Card[0].Card_Flag == false)
        {
            Cards.Message_Text.text = "次に出すあなたの数値は「+2」される。";
        }
    }
    ...
    public void OnClick()
    {
        ...
        //ボタンを押せないように
        Cards.General_Card[0].Use_Card = true;
    ...
    public void General_Win_Lose()
    {
        //自分の処理
        if (PhotonNetwork.player.IsMasterClient)
        {
            GeneralCard_WINorLOSE();
        }
    ...
    public void GeneralCard_WINorLOSE()
    {
        //両方将軍効果がついてない場合
        if (Cards.Own_General_Effect == false && Cards.Other_General_Effect == false)
        {
            switch (Cards.Other_Num)
            {
                case 0:
                    ...
                    //お互いのカードの表示
                    Cards.Owner_Card.text = "あなたは将軍【" + Cards.Own_Num + "】を出しました。";
